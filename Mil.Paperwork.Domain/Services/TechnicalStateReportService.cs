...
{
    public class TechnicalStateReportService : IReportService
    {
        ...
        public bool TryGenerateReport(WriteOffReportData reportData)
        {
            ...
            foreach (var asset in assets)
            {
                ...
                result = report.TryCreate(asset, reportData.WriteOffDate, reportData.Reason);

                if (result)
                {
                    byte[] reportBytes = report.GetReportBytes();

                    var destinationPath = PathsHelper.GetDestinationPath(reportData.DestinationFolder, reportData.WriteOffDate);
