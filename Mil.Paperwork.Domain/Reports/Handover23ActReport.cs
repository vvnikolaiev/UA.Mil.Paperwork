...
namespace Mil.Paperwork.Domain.Reports
{
    internal class Handover23ActReport : IReport
    {
        ...
        private void FillTheFields(IHandoverReportData reportData, WordDocument document)
        {
        ...
        private static void FillAssetsTable(IList<IAssetInfo> assets, WordTable table)
        {
            ...
            var nameCellParameters = new WordCellParameters(Handover23ActHelper.TABLE_FONT_SIZE, WordHorizontalAlignment.Left);
            var cellParameters = new WordCellParameters(Handover23ActHelper.TABLE_FONT_SIZE, WordHorizontalAlignment.Center);
            ...
            for (int i = 0; i < assets.Count; i++)
            {
                ...
                row.GetCell(Handover23ActHelper.COLUMN_INDEX).AddNumber(i + 1, cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_ASSET_NAME).AddText(asset.Name, nameCellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_NOMENCLATURE_CODE).AddText(nomenclatureCode, cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_BATCH_NUMBER).AddText("-", cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_PRICE).AddPrice(asset.Price, cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_COUNT_OUT).AddNumber(asset.Count, cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_CATEGORY_OUT).AddText(category, cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_COUNT_IN).AddNumber(asset.Count, cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_CATEGORY_IN).AddText(category, cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_TOTAL_PRICE).AddPrice(totalPrice, cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_WEAR_N_TEAR).AddText("-", cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_WEAR_N_TEAR_TOTAL).AddText("-", cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_YEAR_MANUFACTURED).AddPrice(asset.YearManufactured, cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_SERIAL_NUMBER).AddText(serialNumber, cellParameters);
                row.GetCell(Handover23ActHelper.COLUMN_PASSPORT_NUMBER).AddText("-", cellParameters);
            }
        ...
        private static void AddSummaryRow(decimal totalSum, int totalCount, WordTable table)
        {
            ...
            textSummaryRow.CreateMergedCell(Handover23ActHelper.COLUMN_INDEX, countMergedColumns);

            textSummaryRow.GetCell(0).AddText(Handover23ActHelper.SummaryRowTotalText, nameCellParameters);
            textSummaryRow.GetCell(Handover23ActHelper.COLUMN_COUNT_IN).AddNumber(totalCount, cellParameters);
            textSummaryRow.GetCell(Handover23ActHelper.COLUMN_COUNT_OUT).AddNumber(totalCount, cellParameters);
            textSummaryRow.GetCell(Handover23ActHelper.COLUMN_TOTAL_PRICE).AddPrice(totalSum, cellParameters);
        }
